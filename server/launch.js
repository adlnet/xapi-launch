...
exports.setup = function(app, DAL)
{
            ...
            DAL.getContentByKey(launch.contentKey, function(err, content)
            {
                DAL.getMedia(launch.mediaKey, function(err, media)
                {
                    ...
                    launchData.credentials = {
                        username: launch.userguid,
                        password: launch.passguid
                    }
    ...
    //test the authorization header against the recorded randomly generated basic autho
    function testAuth(req, launch)
    {
        try
        {
            ...
            //split off the base64 data from the word "Basic "
            header = header.split(" ")[1];
            if (!header) return false;
            //convert back from the base64
            var plain = (new Buffer(header, 'base64')).toString("utf8");
            var user = plain.split(":")[0];
            var pass = plain.split(":")[1];
            if (user == launch.userguid && pass == launch.passguid)
                return true;
            return false;
        }
        catch (E)
    ...
    function validateLaunchSession(cb)
    {
        ...
        return function(req, res, next)
        {
            DAL.getLaunchByGuid(req.params.key, function(err, launch)
            {
                ...
                if (!testAuth(req, launch))
                {
                    res.status(500).send("The auth header is not set correctly");
                    return;
                }
                req.launch = launch;
                DAL.getContentByKey(launch.contentKey, function(err, content)
